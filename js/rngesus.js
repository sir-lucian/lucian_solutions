...
function addPerson() {
    ...
    const inputNameBox = document.getElementById("inputName") ?? undefined;
    if (inputNameBox) {
        inputNameBox.focus();
    }
    checkSorting();
...
function saveToLocalStorage(waitingList) {
    localStorage.setItem("rngesusLucianSolutions", JSON.stringify(waitingList));
}

function loadFromLocalStorage() {
    var storedList = localStorage.getItem("rngesusLucianSolutions") ?? undefined;
    if (storedList) {
        waitingList = JSON.parse(storedList);
    }
}

function clearLocalStorage() {
    localStorage.removeItem("rngesusLucianSolutions");
}

function saveSortingStorage() {
    localStorage.setItem("rngesusSorting", sorting);
}
